...
require_once($GLOBALS['srcdir']."/mdts/includes/report.inc.php");
...
function laboratory_report($pid, $encounter, $cols, $id) {
	...
	try {
		...
		$lab_data = new Laboratory($order_data->lab_id);
...
EOD;
	...
	$processed = ($order_data->date_transmitted > 0)? date('Y-m-d g:ia',strtotime($order_data->date_transmitted)): 'PENDING';
	
	$content = '';
	$content .= do_columns(date('Y-m-d',strtotime($order_data->date_ordered)),'Order Date',$order_data->order_number,'Order Number');
	$content .= do_columns($status_list->getItem($order_data->status),'Order Status',$processed,'Processed Date');
	...
	if ($GLOBALS['wmt::lab_ins_pick']) {
		...
		$content .= do_columns($entby,'Entering Clinician',$billing,'Bill To');
	} else {
		$content .= do_columns($entby,'Entering Clinician',$handle,'Special Handling');
	}
	...
	do_section($header . $content, 'Order Summary', 'max-medium');
	...
	if (!empty($content)) do_section($header . $content, 'Order Diagnosis', 'max-small');
	...
	if ($order_data->order_psc) {
		$pending = FormatDate($order_data->date_pending);
		$content .= do_columns('Yes','PSC Hold Order', $pending, 'Planned Date');
	} elseif (GetSeconds($order_data->date_collected)) {  // returns false if not a valid date
		...
		$collected = FormatDateTime($order_data->date_collected, 'Y-m-d g:ia');
		if (empty($collected)) {
			$collected = FormatDateTime($order_data->date_ordered, 'Y-m-d g:ia');
		}
			...
			if ($order_item->procedure_type == 'pro') {
				...
				if ($codes) {
					...
					while ($profile = sqlFetchArray($result)) {
						...
						$content .= do_columns("","",$description,"Component: ",true);
						$need_blank = true;
					}
				}
			}
	...
	if (!empty($content)) do_section($header . $content, 'Order Requisition', 'max-medium');
...
<?php 
		if (GetSeconds($order_data->reviewed_datetime)) {
			$header = "<tr><td style='min-width:150px'></td><td style='min-width:395px'></td><td style='min-width:140px'></td><td class='w-100'></td></tr>\n";
			$content = do_columns(User::getUserName($order_data->reviewed_id),'Reviewing Provider',User::getUserName($order_data->notified_id),'Notification By');
			$content .= do_columns(FormatDate($order_data->reviewed_datetime),'Reviewed Date',FormatDate($order_data->notified_datetime),'Notified Date');
			$content .= do_columns(false,false,$order_data->notified_person, 'Person Notified');
			...
			do_section($header . $content, 'Review Information', 'max-medium');
		}
	} // end results
?>
<?php 
