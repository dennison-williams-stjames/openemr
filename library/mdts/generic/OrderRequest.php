...
if (!function_exists("makeOrderDocument")) {
	...
	function makeOrderDocument(&$order_data,&$test_list,&$aoe_list) {
		...
		if ($lab['npi'] == 'CPL') {
			...
			$pdf->write1DBarcode($barcode, 'C39', '', 30, 700, 50, 0.4, $style, 'N');
		} 
...
<?php if ($order_data->specimen_fasting) { ?>
				<tr>
					<td colspan=2 width="100%">[fasting] <?php echo $order_data->specimen_fasting; ?></td>
				</tr>
<?php } ?>
<?php if ($order_data->pat_height > 0) { ?>
				<tr>
					<td colspan=2 width="100%">[height] <?php echo $order_data->pat_height ?></td>
				</tr>
<?php } ?>
<?php if ($order_data->pat_weight > 0) { ?>
				<tr>
					<td colspan=2 width="100%">[weight] <?php echo $order_data->pat_weight ?></td>
				</tr>
<?php } ?>
<?php if ($order_data->specimen_volume) { ?>
				<tr>
					<td colspan=2 width="100%">[volume] <?php echo $order_data->specimen_volume ?></td>
				</tr>
<?php } ?>
<?php if ($order_data->specimen_source) { ?>
				<tr>
					<td colspan=2 width="100%">[source] <?php  echo $order_data->specimen_source ?></td>
				</tr>
<?php } 
...
<?php 		
		...
		//if ($lab['npi'] == 'CPL') $bill_list = 'Lab_CPL_Billing';
		$bill_type = ListLook($order_data->request_billing, $bill_list);
...
<table style="width:100%">
	...
	<tr>
		<td style="border:1px solid black"><span style="font-size:.8em;font-weight:bold">
				&nbsp;BILL TYPE:&nbsp;&nbsp;</span><?php echo $bill_type ?>&nbsp;
		</td>
		<td style="border:1px solid black"><span style="font-size:.8em;font-weight:bold">
				&nbsp;INS CODE:&nbsp;&nbsp;</span>
<?php 
		if ( $order_data->request_billing == 'T' ||
				($lab['npi'] == 'CPL' && $order_data->request_billing != 'A' && $order_data->request_billing != 'P') ){
