...
// Global setup
require_once($here."/interface/globals.php");
require_once($GLOBALS['srcdir']."/mdts/mdts.globals.php");

// validate installation
$invalid = "";
	
if (!$lab_id) $invalid .= "No Laboratory Identifier\n";
...
$cat_id = $category['id'];
if (!$cat_id) $invalid .= "Missing $lab_name Document Category\n";
...
if ($lab_data->type == 'quest') {
	...
	if (!file_exists($GLOBALS['srcdir']."/mdts/quest/ResultClient.php")) $invalid .= "Quest Library Missing\n";
	else require_once($GLOBALS['srcdir']."/mdts/quest/ResultClient.php");
}
if ($lab_data->type == 'labcorp') {
	if (!file_exists($GLOBALS['srcdir']."/mdts/labcorp/ResultClient.php")) $invalid .= "Quest Library Missing\n";
	else require_once($GLOBALS['srcdir']."/mdts/labcorp/ResultClient.php");
}
if ($lab_data->type == 'generic') {
	if (!file_exists($GLOBALS['srcdir']."/mdts/generic/ResultClient.php")) $invalid .= "Quest Library Missing\n";
	else require_once($GLOBALS['srcdir']."/mdts/generic/ResultClient.php");
}
...
try {
	...
	if ($from_date) {
		if ($from_date = GetSeconds($from_date)) {
	...
	if ($thru_date) {
		if ($thru_date = GetSeconds($thru_date)) {
			$thru_date = FormatDate($thru_date);
		}
		else {
...
foreach ($messages as $message) {
	...
	if ($order_id) {
		...
		$last_updated = GetSeconds($order_data->report_datetime);
	...
	if (!$pid && !$order_id) {
		...
		$reported = GetSeconds($message->reported_datetime);
	...
	if ($order_data && !$order_data->id) {
		...
		$order_data->facility_id = ($facility_id)? $facility_id : $DUMMY;
		$order_data->lab_id = $lab_id;
		$order_data->pid = ($pid)? $pid : $DUMMY;
		...
		$order_data->date_ordered = FormatDateTime($odate);
		$order_data->date_collected = FormatDateTime($message->specimen_datetime);
		$order_data->date_transmitted = FormatDateTime($message->received_datetime);
	...
	if (count($message->reports) > 0) { // do we have anything to process?
		...
		foreach ($message->reports as $report) {
			...
			$report_data->date_collected = FormatDateTime($message->specimen_datetime);
			...
			if (count($report->results) > 0) { // do we have results for this order?
				foreach ($report->results as $result) {
					...
					if ($debug) {
						...
						echo "Observed Date: " .FormatDateTime($result->observation_datetime). "\n";
					...
					$result_data->date = FormatDateTime($reported); // default to report date
					$result_data->date = FormatDateTime($result->observation_datetime);
...
NOT IMPLEMENTED -------------------- */ 
	...
	/* --------------------------------------------------------------------------- *
 	 ...
 	 * --------------------------------------------------------------------------- */
	$order_data->received_datetime = date('Y-m-d H:i:s');
	$order_data->report_datetime = ($last_updated)? FormatDateTime($last_updated) : date('Y-m-d H:i:s');
