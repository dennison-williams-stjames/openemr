...
if (empty($lab_id)) {
	LogError(E_ERROR, "Save order laboratory identifier missing!!");
	die ();
}
if (empty($pid)) {
	LogError(E_ERROR, "Save order patient identifier missing!!");
	die ();
}
if (empty($encounter)) {
	LogError(E_ERROR, "Save order encounter number missing!!");
	die ();
}
...
/* ------------ SFA ONLY ----------			
	...
	if ($GLOBALS['wmt::lab_portal_notify']) {
		...
		if ($current_portal != 1 && 
				($order_data->portal_flag > 0 && $order_data->reviewed_id > 0 && $order_data->notified_id > 0) ) {
...
if (empty($id)) {
	...
	$order_data->encounter_id = $_SESSION['encounter'];
